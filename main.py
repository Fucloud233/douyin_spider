...
class Task(object):
    ...
    def task(self):
        ...
        if self.numb == 0:
            ...
            # 创建记录的信息
            with open(folder_path + "/" + self.nickname + "_采集数据.csv", 'w', newline='', encoding='gbk',
    ...
    def download(self, folder_path: str, aweme):
        ...
        with open(folder_path + "/" + self.nickname + "_采集数据.csv", 'a', newline='', encoding='gbk',
                  errors='ignore') as csvfile:
            fieldnames = ['aweme_id', '时间', 'title', '格式', '收藏', '评论', '点赞', '分享', 'share_url']
            writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
        ...
        if aweme['images'] == None:
            ...
            video = get(url)
            # 添加基础路径
            with open(folder_path + '/' + aweme["author"]["nickname"] + "/video/" + aweme["aweme_id"] + '.mp4', 'wb') as f:
